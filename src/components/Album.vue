...
import { QQMusicResponse, AlbumInfo, SongInfo, Song, Singer } from '@/common/interface';
...
export default class Album extends Vue{
    ...
    album:AlbumInfo|null = null;
    menuedIndex = 0;
    mid = this.$route.params.id;

    get albumImgUrl():string{
    ...
    play(index:number){
        const list:Song[] = this.album.list.map<Song>((item:SongInfo)=>({
            id: item.songid,
            mid: item.songmid,
            name: item.songorig,
            singer: item.singer,
        ...
        this.playService.setPlayList({index,list});
    ...
    getSingerStr(val:Singer[]|string):string{
    ...
    created(){
        this.api.getAlbum(this.mid).subscribe((response:QQMusicResponse<AlbumInfo>)=>{
